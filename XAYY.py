...
def banner():
	...
	sol().print(cik2)
	ban='''
•   AUTHOR  : LORD-XD WHATSAPP : 083876399762 •
•   _____ _______  ____________________         •
•   __  // /__   |/  /__  __ )__  ____/         •
•   _  // /__  /|_/ /__  __  |_  /_             •
•   /__  __/  /  / / _  /_/ /_  __/             •
•     /_/  /_/  /_/  /_____/ /_/ VERSION 4.0.6 •
•                                               •
•   GITHUB : HTTPS://GITHUB.COM/LORD-XD   •
'''
...
def login_lagi334():
	try:
		banner()
		___kambing___ = input('[|] Masukkan Cookies : ')
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (Linux; Android10; Samsung J2 Build/MMB29M) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.92 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":___kontol___}) 
		find_token = re.search("(EAAG\w+)", data.text)
		ken=open(".token.txt", "w").write(find_token.group(1))
		cok=open(".cok.txt", "w").write(___kambing___)
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/4You-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/PakYou-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/Pucek-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/4You-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/4YOU-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/4YOU-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/PACKYOU-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
		mer = os.listdir('/sdcard/4YOU-DATA/OK')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		...
		lol = {}
		for isi in my_files:
			try:hem = open('/sdcard/PACKYOU-DATA/CP/'+isi,'r').readlines()
			except:
				try:hem = open('/sdcard/4YOU-DATA/OK/'+isi,'r').readlines()
				except:continue
			cih+=1
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/PACKYOU-DATA/1CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/4YOU-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] ENTER TOTAL ID LIMIT [40]')
		try:
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/PACKYU-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/4YOU-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/4YOU-DATA/DUMP')
	except:pass
	login()
